# -*- coding: utf-8 -*-
"""Callbacks for PPO training in the transfer quadrotor overlay."""
...
try:  # SB3 has moved this helper across versions.
...
class ActionStdCallback(BaseCallback):
    """Control the PPO diagonal Gaussian action standard deviation.

    ``std_mode='learnable'`` keeps SB3's default learnable log_std parameter.
    ``std_mode='fixed'`` freezes log_std at ``log_std_init``.
    ``std_mode='anneal'`` freezes log_std and linearly moves it from
    ``log_std_init`` to ``log_std_final`` over ``total_timesteps``.

    The values are in normalized action units, because the environment action
    space is Box([-1, -1], [1, 1]).  The training script prints the physical
    interpretation in N and degrees.
    """
...
class RewardOrientedBestModelCallback(BaseCallback):
    """Evaluate periodically and save the best model by a task-oriented metric.

    The default metric is ``task_return_mean``: the undiscounted sum of
    ``info['task_reward']`` over held-out episodes.  This deliberately ignores
    the lambda smoothness penalty so that different lambda policies are compared
    on the same navigation objective.
    """
    ...
    def _make_eval_env(self):
        ...
        if train_vecnorm is not None:
            ...
            if sync_envs_normalization is not None:
                ...
                # Conservative fallback for common SB3 versions.
