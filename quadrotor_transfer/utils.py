# -*- coding: utf-8 -*-
"""Utility functions for reduced-to-full quadrotor transfer experiments.

The utilities here deliberately stay outside the upstream ``safe_control_gym``
package.  They handle run naming, artifact discovery, scalar summaries, JSON
logging, and stdout/stderr teeing for reproducible debug logs.
"""
...
def safe_float_name(value: float) -> str:
    """Return a filename-safe representation of a float."""
    return f"{float(value):g}".replace("-", "m").replace(".", "p")


def lambda_to_name(lambda_penalty: float) -> str:
    """Return the canonical lambda directory/file stem."""
    return f"lambda_{safe_float_name(lambda_penalty)}"


def now_timestamp() -> str:
    """Return a local timestamp suitable for folder names."""
...
def json_default(value):
    """JSON serializer for pathlib/numpy values."""
...
class TeeStream:
    """File-like object that writes to several streams.

    Use this to capture verbose SB3/PyBullet prints without losing terminal logs.
    """
...
def setup_tee_logging(log_path: Union[Path, str]):
    """Redirect stdout/stderr to terminal plus a file.

    Returns the opened file handle.  Keep it alive until the program exits.
    """
    log_path = Path(log_path)
    log_path.parent.mkdir(parents=True, exist_ok=True)
    log_file = open(log_path, "a", buffering=1, encoding="utf-8")
    sys.stdout = TeeStream(sys.__stdout__, log_file)  # type: ignore[assignment]
    sys.stderr = TeeStream(sys.__stderr__, log_file)  # type: ignore[assignment]
    return log_file
...
def scalar(value) -> float:
    """Convert scalar-like arrays/lists to a Python float."""
    return float(np.asarray(value, dtype=float).reshape(-1)[0])


def as_1d_float_array(values: Union[Sequence[float], float], *, length: int = 2, name: str = "value") -> np.ndarray:
    """Convert a scalar or length-N sequence into a 1D array of requested length."""
...
def std_to_log_std(std_values: Union[Sequence[float], float], *, length: int = 2, name: str = "std") -> np.ndarray:
    """Convert positive standard deviations to log standard deviations."""
...
def parse_optional_float_list(text: Optional[str]) -> Optional[list[float]]:
    """Parse comma-separated floats if a CLI argument is supplied."""
...
def update_run_index(
    *,
    ...
    """Write/update a lightweight by-lambda index without deleting any run."""
...
def _candidate_model_from_record(record: dict, selector: str) -> tuple[Optional[Path], Optional[Path], Optional[Path]]:
    ...
    else:  # latest means latest run's final if available, falling back to best.
        model_keys = ["final_model_path", "best_model_path"]
        vec_keys = ["final_vecnormalize_path", "best_vecnormalize_path"]
...
def find_policy_artifacts(
    ...
    """Find PPO artifacts for a lambda without requiring a fragile fixed layout.

    New layout preferred by this package:
        models/transfer_quadrotor_reduced_policies/runs/<run_id>/best_model.zip
        models/transfer_quadrotor_reduced_policies/runs/<run_id>/final_model.zip
        models/transfer_quadrotor_reduced_policies/by_lambda/lambda_X/latest.json

    Older layout from the first overlay is still supported.
    """
    selector = selector.lower()
    ...
    # Older stable layout: <root>/lambda_0/final_model.zip.
    ...
    # Flat layout: <root>/lambda_0_policy.zip.
    for variant in variants:
    ...
    # Last-resort recursive search, excluding checkpoint folders unless no better option.
    if not candidates:
    ...
    # Pick most recently modified run/model.
...
def describe_action_std(std_normalized: Union[Sequence[float], float], theta_max_rad: float, delta_thrust_max: float) -> dict:
    """Return physical interpretation of normalized PPO action std."""
